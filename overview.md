...
In general IAMs can be divided into two categories, those that are highly aggregated and whose core problem is the conduct of global, century-scale, cost-benefit analysis (examples include DICE, FUND, and PAGE), and higher resolution models whose core problem is understanding the details of human and physical Earth system interactions. GCAM is a member of the latter class of more highly-resolved IA models.

IA models like GCAM do not predict the future. They instead provide "what if" scenarios, quantifying the implications of different possible future conditions. GCAM reads in external "scenario assumptions" about key drivers such as population, economic activity, technology, and policies. GCAM can then assess the implications of these assumptions on, for example, commodity prices, energy use, land use, water use, emissions, and concentrations. 
