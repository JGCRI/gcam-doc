...
There is a method in the GCAM data system that allows users to selectively specify both new vintage emissions factors and existing vintage retrofits (base-year or future) for technologies, replacing the current smooth GDP control for those technologies, without changing any model code. The user just needs to place any number of CSV input files into a folder, and the system will generate XML input files that will use this new data to replace any previous emission factors and/or GDP control functions. Two options are available. The main option is to specify a New Source Performance Standard (NSPS) for a technology, which is the emission factor that newly built technologies should have (often specified by air pollution control regulations). The NSPS is functionally applied as an emission factor. 

Retrofits can also be specified (this is only applicable to technologies with a lifetime > 1 period). Retrofits are implemented as a linear-control object such that the emissions factor linearly declines from its value in specified starting year to the specified end point, for the specified technology vintage. The code automatically removes the existing GDP control object for any technology for which either a NSPS or retrofit is specified.
