...
There has been a significant data method update for the GCAM AgLU (Agriculture and Land Use) sectors in GCAM v7, which is described in detail in [CMP #360](cmp/360-AgLU_data_and_methods.pdf). An additional package (`gcamfaostat`) is developed to facilitate the processing of data from FAOSTAT in a transparent, traceable, and consistent manner. This update encompasses the following key advancements:
